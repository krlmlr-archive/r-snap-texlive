\documentclass[UKenglish]{article}
\usepackage[xyz,prova,pointsonright,autopstoff,param]{esami}%%%  numeric version
%%\usepackage[mg,prova,pointsonright,autopstoff,param]{esami}%%%  parametric version
\date{2013/4/8}%%% it's necessary for the random number generation, but can be any date
\FPeval\seme{2010}%%% or what you like
\randomi=\seme

\begin{document}

% +=======================================================+
% |            UNCOMMENT THE PART YOU NEED                |
% | AND WRITE THE NAME OF THE FILE IN \def\esercizio{}    |
% +=======================================================+

% +=============================+
% |      FOR TEST               |
% +=============================+

\def\esercizio{test11}
\makeatletter
\ifes@param
\begin{center} {\textbf{\esercizio -p}}\vspace{5mm}\end{center}
\else
\begin{center} {\textbf{\esercizio}}\vspace{5mm}\end{center}
\fi
\begin{shortquiz}
\begin{questions}
\selectallproblems{\esercizio}

\end{questions}
\end{shortquiz}


% +=============================+
% |      FOR EXERCISES          |
% +=============================+

%\def\esercizio{}
%\makeatletter
%\ifes@param
%\begin{center} {\textbf{\esercizio -p}}\vspace{5mm}\end{center}
%\else
%\begin{center} {\textbf{\esercizio}}\vspace{5mm}\end{center}
%\fi
%\selectallproblems{\esercizio}
\end{document}

